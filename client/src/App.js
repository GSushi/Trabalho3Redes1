...
function App() {
  ...
  function connect() {
    setConnected(true);
    socketRef.current = io.connected("/");
    socketRef.current.emmit("join server", username);
    socketRef.current.emmit("join room", "general", (messages) => roomJoinCallback(messages, "general"));
